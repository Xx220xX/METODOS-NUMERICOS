...
int main() {
    gmConfig.sizeBlock = 2;
    style = 2;
    printa = _2;
    SetConsoleTitleA("          TETRIS");
    editaCursor(0, 1);
    Record atualR[10] = {0};
    int scrxT = 145, scryT = 56, nextX0 = scrxT * 2 / 3.0 + 3, nextY0 = 2;
    gmConfig.rec = atualR;
    for (;;) {
    ...
    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++) {
            if (mat[i][j] && mat[i][j]!=-5) {
                ...
                for(l=0;l<gmConfig.sizeBlock;l++){
                    for(l2=0;l2<gmConfig.sizeBlock;l2++){
                        gotoxy(x+l2, y+l);
                        printf(" ");
                    }}
...
void jogo(Record *atualR, int *scrxT, int *scryT, int *nextX0, int *nextY0) {
    ...
    int max, pnts = 0, l0 = 0, nivel = gmConfig.nivel;
...
void ajuste(unsigned int *t, int nivel) {
    *t = 0.0013 * nivel * nivel * nivel * nivel - 0.1393 * nivel * nivel * nivel + 5.9579 * nivel * nivel -
         122.24 * nivel + 1114.9;
}
